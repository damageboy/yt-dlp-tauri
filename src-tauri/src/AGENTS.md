...
| `aria2c_rpc.rs` | Owns local authenticated aria2c RPC lifecycle, declared/streamed response limits and progress projection; See change: aria2c-rpc-progress. |
...
| `aria2c_rpc_tests.rs` | Uses blocking accepted sockets; tolerates client timeout disconnects and Windows connection-refusal timing; See change: fork-release-builds. Verifies RPC progress, queue shutdown, transport and session lifecycle; See change: aria2c-rpc-progress. |
